...
async def get_trader_from_session(request: Request, db: AsyncSession = Depends(get_db)) -> Trader:
    ...
    if not payload or "sub" not in payload:
        # Try refresh before bouncing the user.
        refresh = request.session.get("refresh_token")
        refresh_payload = verify_token(refresh) if refresh else None
...
async def settings_categories_page(
    ...
    # Organize into a flat list ordered: root categories first, then their children
    cats_flat = result["categories"]
    child_map: dict = {}
    for c in cats_flat:
        if c.get("parentId"):
            child_map.setdefault(c["parentId"], []).append(c)
    organized = []
    for c in cats_flat:
        if c.get("parentId") is None:
            organized.append({**c, "indent": 0})
            for child in child_map.get(c["sourceId"], []):
                organized.append({**child, "indent": 1})
    selected = set(request.session.get("allowed_category_ids") or [])
    return templates.TemplateResponse(
...
async def settings_categories_save(
    request: Request,
    trader: Trader = Depends(get_trader_from_session),
):
    from app.core.admin_client import admin_client
    ...
    category_ids = [int(v) for v in form.getlist("category_ids")]
    try:
        await admin_client.update_allowed_categories(
            access_token=backend_token,
            api_key=trader.api_key or "",
        ...
        return RedirectResponse(url=f"/settings/categories?error={e}", status_code=303)
    request.session["allowed_category_ids"] = category_ids
    return RedirectResponse(url="/settings/categories?success=Saved", status_code=303)
